...
from datasets import load_dataset  # Keep if planning to use HF datasets
...
from diffusers import (
    AutoencoderKL,
    DDPMScheduler,
    StableDiffusionPipeline, # Added for saving pipeline
...
from huggingface_hub import (
    ...
    Repository,
    create_repo,
    login, # Added for potential login needs
    whoami,
)
...
try:
    ...
    logger.warning("Could not set multiprocessing sharing strategy 'file_system'. This might be okay depending on the OS.")
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="Fine-tune Stable Diffusion for Document Inpainting with Text Conditioning.")

    # --- Paths and Data ---
    data_group = parser.add_argument_group("Data and Paths")
    data_group.add_argument("--pretrained_model_name_or_path", type=str, required=True, help="Path to pretrained Stable Diffusion model or model identifier from Hugging Face Hub.")
    data_group.add_argument("--revision", type=str, default=None, help="Revision of pretrained model identifier from Hugging Face Hub.")
    data_group.add_argument("--non_ema_revision", type=str, default=None, help="Revision of non-EMA model identifier (if applicable). Deprecated: Use --variant=non_ema.")
    data_group.add_argument("--cache_dir", type=str, default=None, help="Directory to cache pretrained models.")
    data_group.add_argument("--train_data_csv", type=str, required=True, help="Path to the CSV file containing image paths (relative to data source). Must contain a 'path' column.")
    data_group.add_argument("--ocr_data_root", type=str, required=True, help="Root directory/prefix for OCR JSON files (relative to data source).")
    data_group.add_argument("--data_source", type=str, default="pcache", choices=["local", "minio", "pcache"], help="Source for loading images and OCR data.")
    data_group.add_argument("--font_path", type=str, default=DEFAULT_FONT_PATH, help="Path to the TTF font file for rendering text.")
    data_group.add_argument("--output_dir", type=str, default="diffute-model-output", help="The output directory where the model predictions and checkpoints will be written.")

    # --- Model Configuration ---
    model_group = parser.add_argument_group("Model Configuration")
    model_group.add_argument("--resolution", type=int, default=DEFAULT_RESOLUTION, help="The resolution for input images and masks.")
    model_group.add_argument("--crop_scale", type=int, default=DEFAULT_CROP_SCALE, help="The size of the square crop region around the text box.")
    model_group.add_argument("--trocr_model_name", type=str, default=DEFAULT_TROCR_MODEL, help="TrOCR model identifier for text feature extraction.")
    model_group.add_argument("--center_crop", action="store_true", help="Whether to center crop images before resizing.") # Note: Current dataset doesn't use center_crop explicitly
    model_group.add_argument("--random_flip", action="store_true", help="Whether to randomly flip images horizontally.") # Note: Current dataset doesn't use random_flip

    # --- Training Parameters ---
    train_group = parser.add_argument_group("Training Parameters")
    train_group.add_argument("--train_batch_size", type=int, default=16, help="Batch size (per device) for the training dataloader.")
    train_group.add_argument("--num_train_epochs", type=int, default=100, help="Total number of training epochs to perform.")
    train_group.add_argument("--max_train_steps", type=int, default=None, help="Total number of training steps to perform. If provided, overrides num_train_epochs.")
    train_group.add_argument("--gradient_accumulation_steps", type=int, default=1, help="Number of updates steps to accumulate before performing a backward/update pass.")
    train_group.add_argument("--gradient_checkpointing", action="store_true", help="Enable gradient checkpointing to save memory.")
    train_group.add_argument("--learning_rate", type=float, default=1e-5, help="Initial learning rate (after the potential warmup period) to use.")
    train_group.add_argument("--scale_lr", action="store_true", default=False, help="Scale the learning rate by the number of GPUs, gradient accumulation steps, and batch size.")
    train_group.add_argument("--lr_scheduler", type=str, default="constant", help='The scheduler type to use. Choose between ["linear", "cosine", "cosine_with_restarts", "polynomial", "constant", "constant_with_warmup"]')
    train_group.add_argument("--lr_warmup_steps", type=int, default=500, help="Number of steps for the warmup in the lr scheduler.")
    train_group.add_argument("--use_8bit_adam", action="store_true", help="Use 8-bit AdamW optimizer.")
    train_group.add_argument("--adam_beta1", type=float, default=0.9, help="The beta1 parameter for the Adam optimizer.")
    train_group.add_argument("--adam_beta2", type=float, default=0.999, help="The beta2 parameter for the Adam optimizer.")
    train_group.add_argument("--adam_weight_decay", type=float, default=1e-2, help="Weight decay to use.")
    train_group.add_argument("--adam_epsilon", type=float, default=1e-08, help="Epsilon value for the Adam optimizer.")
    train_group.add_argument("--max_grad_norm", default=1.0, type=float, help="Max gradient norm.")
    train_group.add_argument("--use_ema", action="store_true", help="Use Exponential Moving Average for model weights.")
    train_group.add_argument("--ocr_confidence_threshold", type=float, default=DEFAULT_OCR_CONFIDENCE_THRESHOLD, help="Minimum confidence score to consider an OCR detection.")

    # --- Performance & Hardware ---
    perf_group = parser.add_argument_group("Performance and Hardware")
    perf_group.add_argument("--dataloader_num_workers", type=int, default=0, help="Number of subprocesses to use for data loading.")
    perf_group.add_argument("--mixed_precision", type=str, default=None, choices=["no", "fp16", "bf16"], help="Whether to use mixed precision. Choose between fp16 and bf16 (bfloat16). Bf16 requires PyTorch >= 1.10 and an Nvidia Ampere GPU.")
    perf_group.add_argument("--enable_xformers_memory_efficient_attention", action="store_true", help="Enable xformers memory-efficient attention.")
    perf_group.add_argument("--allow_tf32", action="store_true", help="Allow TF32 on Ampere GPUs (may decrease precision slightly for speed).")
    perf_group.add_argument("--seed", type=int, default=None, help="A seed for reproducible training.")
    perf_group.add_argument("--local_rank", type=int, default=-1, help="For distributed training: local_rank")

    # --- Logging, Checkpointing & Hub ---
    log_group = parser.add_argument_group("Logging, Checkpointing and Hub")
    log_group.add_argument("--logging_dir", type=str, default="logs", help="TensorBoard log directory (subdirectory under output_dir).")
    log_group.add_argument("--report_to", type=str, default="tensorboard", help='The integration to report results to (e.g., "tensorboard", "wandb").')
    log_group.add_argument("--checkpointing_steps", type=int, default=500, help="Save a checkpoint of the training state every X updates steps.")
    log_group.add_argument("--checkpoints_total_limit", type=int, default=None, help="Max number of checkpoints to store. Old ones are deleted.")
    log_group.add_argument("--resume_from_checkpoint", type=str, default=None, help="Whether to resume training from latest checkpoint ('latest') or a specific checkpoint folder name.")
    log_group.add_argument("--push_to_hub", action="store_true", help="Whether to push the model to the Hugging Face Hub.")
    log_group.add_argument("--hub_token", type=str, default=None, help="The token to use to push to the Model Hub.")
    log_group.add_argument("--hub_model_id", type=str, default=None, help="The name of the repository on the Hub.")
    log_group.add_argument("--hub_organization", type=str, default=None, help="The organization name on the Hub (optional).")
    ...
    if not os.path.exists(args.font_path):
        logger.warning(f"Specified --font_path '{args.font_path}' does not exist. Text rendering might fail.")

    # Check data source requirements
    if args.data_source == "minio" and not all([MINIO_ENDPOINT, MINIO_ACCESS_KEY, MINIO_SECRET_KEY, MINIO_BUCKET]):
        raise ValueError("MinIO data source selected, but required environment variables (MINIO_ENDPOINT, MINIO_ACCESS_KEY, MINIO_SECRET_KEY, MINIO_BUCKET) are not fully set.")
    if args.data_source == "pcache" and not PCACHE_AVAILABLE:
        raise ValueError("pcache data source selected, but pcache_fileio library is not installed.")
    if args.data_source == "pcache" and not OSS_PCACHE_ROOT_DIR:
        raise ValueError("pcache data source selected, but OSS_PCACHE_ROOT_DIR environment variable is not set.")
...
def get_minio_client() -> Optional[Minio]:
    ...
    if not all([MINIO_ENDPOINT, MINIO_ACCESS_KEY, MINIO_SECRET_KEY, MINIO_BUCKET]):
        logger.warning("MinIO environment variables not fully set. MinIO client not created.")
        return None
    try:
        client = Minio(
...
def download_content_pcache(relative_file_path: str) -> Optional[bytes]:
    ...
    try:
        ...
        logger.error(f"Error processing file {relative_file_path} using pcache_fileio: {e}", exc_info=True)
        return None
...
def load_json_from_bytes(content: bytes, file_identifier: str) -> Optional[Dict]:
    ...
    try:
        ...
        if 'document' not in ocr_data or not isinstance(ocr_data['document'], list):
            logger.warning(f"Invalid OCR JSON format for {file_identifier}: missing 'document' list.")
            return None
        return ocr_data
        ...
        logger.error(f"Unexpected error parsing OCR JSON for {file_identifier}: {e}", exc_info=True)
        return None
...
def draw_text_on_image(text: str, font_path: str) -> np.ndarray:
    ...
    try:
        ...
        logger.error(f"Font file '{font_path}' not found or invalid. Cannot render text.")
    ...
    img = Image.new('RGB', (max(img_width, 1), max(img_height, 1)), color='white') # Ensure min size 1x1
    draw = ImageDraw.Draw(img)
    draw.text(pos, text, font=font, fill='black')

    return np.array(img) # Return RGB NumPy array

def process_bounding_box(location: List[float], image_shape: Tuple[int, int]) -> List[int]:
    ...
    # Ensure min < max
    if x_max_int <= x_min_int: x_max_int = x_min_int + 1
    if y_max_int <= y_min_int: y_max_int = y_min_int + 1
...
def generate_mask_from_box(mask_shape: Tuple[int, int], box: List[int]) -> np.ndarray:
    ...
    if x_max > x_min and y_max > y_min:
        ...
        logger.warning(f"Invalid rectangle dimensions for mask: {box} within shape {(width, height)}")
...
class DocumentInpaintingDataset(Dataset):
    ...
    def __init__(self,
        ...
        logger.info(f"Initialized dataset with {self.num_images} images from {self.data_source}.")
    ...
    def _load_image_paths(self) -> List[str]:
        ...
        try:
            ...
            logger.error(f"Error loading image paths from {self.data_csv_path}: {e}", exc_info=True)
            raise
    ...
    def _process_ocr_data(self, ocr_data: Dict, image_path: str) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            if not all(col in ocr_pd.columns for col in ['text', 'score', 'box']):
                logger.warning(f"OCR data frame missing required columns for {image_path}")
                return None
            ocr_pd_filtered = ocr_pd[ocr_pd['score'] > self.ocr_confidence_threshold].copy()
            if ocr_pd_filtered.empty:
                logger.warning(f"No high-confidence OCR results found for {image_path}")
                return None
            return ocr_pd_filtered
        except Exception as e:
            logger.error(f"Error processing OCR data for {image_path}: {e}", exc_info=True)
            return None

    def _get_crop_coordinates(self, box: List[int], image_shape: Tuple[int, int]) -> Tuple[int, int]:
        ...
        max_start_x = min(current_w - self.crop_scale, x1) # Must start before box starts
        ...
        try:
            x_s = np.random.randint(min_start_x, max_start_x + 1) if max_start_x >= min_start_x else min_start_x
            y_s = np.random.randint(min_start_y, max_start_y + 1) if max_start_y >= min_start_y else min_start_y
        except ValueError as ive:
            logger.warning(f"Issue calculating crop start: {ive}. Using fallback. Box: {box}, Img Size: {image_shape}")
            x_s = max(0, current_w - self.crop_scale) # Fallback: crop from bottom-right
    ...
    def __getitem__(self, index):
        ...
        try:
            ...
            if image_content is None: raise FileNotFoundError(f"Image not found: {image_path}")
            if ocr_content is None: raise FileNotFoundError(f"OCR not found: {ocr_path}")

            instance_image = load_image_from_bytes(image_content) # RGB NumPy array
            ocr_data = load_json_from_bytes(ocr_content, ocr_path)

            if instance_image is None: raise ValueError("Image decoding failed")
            if ocr_data is None: raise ValueError("OCR JSON parsing failed")
            ...
            if ocr_pd_filtered is None: raise ValueError("No valid OCR data found")
            ...
            try:
                ...
                raw_location = [min(x_coords), min(y_coords), max(x_coords), max(y_coords)]
            except (IndexError, TypeError):
                raise ValueError(f"Invalid 'box' format in OCR data: {box_coords}")

            location_box = process_bounding_box(raw_location, original_shape_hw) # [x_min, y_min, x_max, y_max] ints

            # 3. Generate Mask and Masked Image (Original Size)
            mask = generate_mask_from_box((original_shape_hw[1], original_shape_hw[0]), location_box) # HW, 0/255
            ...
            augmented = self.transform_to_tensor(image=mask_final[:, :, np.newaxis]) # Add channel dim
            ...
            logger.error(f"Error processing data at index {index} (path: {image_path}): {e}", exc_info=True)
...
def get_full_repo_name(model_id: str, organization: Optional[str] = None, token: Optional[str] = None) -> str:
    ...
    if organization is None:
        if token is None:
             raise ValueError("No organization provided and no token found. Please login using `huggingface-cli login` or pass --hub_token.")
        try:
...
def tensor2im(input_image: torch.Tensor, imtype=np.uint8) -> np.ndarray:
    ...
    if not isinstance(input_image, np.ndarray):
        ...
        image_numpy = image_tensor[0].cpu().float().numpy() # Select first image if batch
...
def load_models_and_scheduler(args):
    ...
    processor = TrOCRProcessor.from_pretrained(args.trocr_model_name, cache_dir=args.cache_dir)
    trocr_model = VisionEncoderDecoderModel.from_pretrained(args.trocr_model_name, cache_dir=args.cache_dir).encoder
    ...
    unet = UNet2DConditionModel.from_pretrained(
        ...
        revision=args.non_ema_revision or args.revision, # Use non_ema if specified, else main revision
...
def setup_optimizer_and_scheduler(args, unet, accelerator):
    ...
    logger.info("Optimizer initialized. LR scheduler will be created after dataloader preparation.")
    return optimizer
    ...
    if version.parse(accelerate.__version__) >= version.parse("0.16.0"):
        def save_model_hook(models, weights, output_dir):
            ...
            if models: # Should contain the UNet
                ...
                if weights: weights.pop() # Check if weights list is not empty

        def load_model_hook(models, input_dir):
            logger.debug(f"Load hook called for {input_dir}")
            if args.use_ema and ema_unet is not None:
                logger.info(f"Loading EMA UNet from {os.path.join(input_dir, 'unet_ema')}")
            ...
            if models: # Should contain the UNet placeholder
                ...
                models[0].register_to_config(**load_model.config) # Use the first model in the list
        ...
        logger.warning("Accelerator version < 0.16.0. Custom save/load hooks not registered. Checkpoint saving might use standard PyTorch format.")


# === Training Step ===
def train_step(batch, unet, vae, trocr_model, processor, noise_scheduler, weight_dtype, accelerator):
    ...
    with torch.no_grad():
        ...
        ocr_embeddings = ocr_feature.last_hidden_state # (batch_size, seq_len, hidden_size)
    ...
    mask = F.interpolate(
        batch["masks"].to(accelerator.device, dtype=weight_dtype), # Ensure mask is on device
...
# === Main Training Loop ===
def training_loop(args, accelerator, unet, vae, trocr_model, processor, noise_scheduler, optimizer, lr_scheduler, train_dataloader, ema_unet, weight_dtype):
    ...
    if args.resume_from_checkpoint:
        if args.resume_from_checkpoint != "latest":
            ...
            if not dirs:
                logger.warning("Resume 'latest' requested, but no checkpoint found. Starting fresh.")
        ...
        if path and os.path.isdir(os.path.join(args.output_dir, path)):
            ...
            try:
                ...
                logger.info(f"Resumed training from global step {global_step}, epoch {first_epoch}, step {resume_step}.")
            except Exception as e:
                 logger.error(f"Failed to load checkpoint state from {resume_path}: {e}. Starting fresh.")
                 global_step = 0
                 first_epoch = 0
                 resume_step = 0
                 args.resume_from_checkpoint = None # Disable resume on failure
        else:
            logger.warning(f"Checkpoint '{args.resume_from_checkpoint}' not found or invalid. Starting fresh.")
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if batch is None:
                logger.warning(f"Skipping step {step} in epoch {epoch} due to invalid batch.")
            ...
            with accelerator.accumulate(unet):
                try:
                    loss = train_step(batch, unet, vae, trocr_model, processor, noise_scheduler, weight_dtype, accelerator)
                except ValueError as e: # Catch errors from train_step (e.g., TrOCR failure)
            ...
            if accelerator.sync_gradients:
                ...
                # Logging
                if global_step % args.checkpointing_steps == 0: # Log less frequently than every step
                     logs = {
                     ...
                     logs = {"inst_loss": loss.detach().item(), "lr": lr_scheduler.get_last_lr()[0]}
                     progress_bar.set_postfix(**logs)
...
def save_final_model(args, accelerator, unet, vae, ema_unet, weight_dtype):
    ...
    if accelerator.is_main_process:
        ...
        try:
            ...
            logger.warning(f"Could not save final pipeline (may require components like text_encoder): {e}")
             ...
             if os.path.exists(gitignore_path):
                 try:
                     ...
                     with open(gitignore_path, "w") as f:
                         for line in lines:
                             if not line.strip().startswith(("step_*", "epoch_*", "checkpoint-*")):
                                 f.write(line)
...
def main():
    ...
    if accelerator.is_main_process and args.push_to_hub:
        if args.hub_model_id is None:
            repo_name = get_full_repo_name(Path(args.output_dir).name, organization=args.hub_organization, token=args.hub_token)
        else:
        ...
        with open(os.path.join(args.output_dir, ".gitignore"), "a+") as gitignore:
             ...
             if "checkpoint-*/" not in content: gitignore.write("checkpoint-*/\n")
    ...
    if args.enable_xformers_memory_efficient_attention:
        if is_xformers_available():
            try:
                ...
                logger.warning(f"Could not enable xformers: {e}. Proceeding without it.")
        else:
            logger.warning("xformers not available. Install for potential memory savings.")
    ...
    if args.max_train_steps is None:
        ...
        logger.info(f"Using provided max_train_steps: {args.max_train_steps}. Effective epochs: {args.num_train_epochs}")
    ...
    logger.info(f"Moved VAE and TrOCR to {accelerator.device} with dtype {weight_dtype}")
    ...
    # --- Training ---
    training_loop(
        args, accelerator, unet, vae, trocr_model, processor,
        noise_scheduler, optimizer, lr_scheduler, train_dataloader,
        ema_unet, weight_dtype
    )
